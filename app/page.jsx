...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .btn-print{position:fixed;top:16px;right:16px;background:#FFBA00;color:#000;border:none;padding:10px 24px;border-radius:8px;font-family:'DM Sans',sans-serif;font-weight:700;font-size:14px;cursor:pointer;z-index:999;box-shadow:0 4px 12px rgba(0,0,0,.2)}
  ...
  .logo-box{width:40px;height:40px;background:#FFBA00;border-radius:7px;display:flex;align-items:center;justify-content:center;margin-right:12px}
  .logo-txt{font-family:'Bebas Neue';font-size:18px;color:#000}
  .doc-type{font-family:'Bebas Neue';font-size:26px;color:#FFBA00;letter-spacing:1px}
  .doc-date{font-size:10px;color:#888;margin-top:2px;text-align:right}
  .hero{padding:28px 32px;border-bottom:3px solid #FFBA00;display:flex;gap:24px;align-items:flex-start}
  .hero-info{flex:1}
  .product-name{font-family:'Bebas Neue';font-size:44px;color:#111;letter-spacing:1px;line-height:1;margin-bottom:10px}
  .badges{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px}
  .badge{padding:4px 12px;border-radius:20px;font-size:10px;font-weight:700;letter-spacing:.5px}
  .b-gold{background:#FFBA00;color:#000}
  ...
  .concept-lbl{font-size:9px;font-weight:700;color:#888;letter-spacing:1.5px;margin-bottom:6px}
  .concept-txt{font-size:13px;line-height:1.7;color:#333;white-space:pre-wrap}
  .hero-foto{width:200px;height:170px;border-radius:10px;overflow:hidden;flex-shrink:0;border:2px solid #eee}
  .hero-foto img{width:100%;height:100%;object-fit:cover}
  .hero-foto-empty{width:100%;height:100%;background:#f5f5f5;display:flex;align-items:center;justify-content:center;color:#ccc;font-size:28px}
  .info-grid{display:grid;grid-template-columns:1fr 1fr 1fr;border-bottom:1px solid #eee}
  .info-cell{padding:14px 20px;border-right:1px solid #eee}
  .info-cell:last-child{border-right:none}
  .info-lbl{font-size:9px;font-weight:700;color:#888;letter-spacing:1.5px;margin-bottom:4px}
  ...
  .sec-title{font-family:'Bebas Neue';font-size:17px;letter-spacing:1px}
  .sec-content{font-size:12px;line-height:1.9;color:#333;white-space:pre-wrap}
  .two-col{display:grid;grid-template-columns:1fr 1fr;gap:24px}
  .fotos-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:6px}
  .foto-item{border-radius:8px;overflow:hidden;aspect-ratio:4/3;border:1px solid #eee}
  .foto-item img{width:100%;height:100%;object-fit:cover}
  .footer{background:#111;color:#555;padding:14px 32px;display:flex;justify-content:space-between;align-items:center;font-size:10px}
  .footer-brand{font-family:'Bebas Neue';font-size:14px;color:#FFBA00;letter-spacing:1px}
  .conf{background:#FFBA00;color:#000;padding:3px 10px;border-radius:4px;font-weight:700;font-size:9px;letter-spacing:1px}
</style>
</head>
<body>
<button class="btn-print no-print" onclick="window.print()">⬇ Descargar / Imprimir PDF</button>

<div class="header">
  <div style="display:flex;align-items:center">
    <div class="logo-box"><span class="logo-txt">BP</span></div>
    <div><div class="brand-name">BIG★PONS</div><div class="brand-sub">AMERICAN CLASSIC BURGERS</div></div>
  </div>
  <div><div class="doc-type">FICHA DE PRODUCTO</div><div class="doc-date">${fecha}</div></div>
</div>
...
<div class="hero">
  <div class="hero-info">
    ...
    <div class="badges">
      ${idea.segmento ? `<span class="badge b-gold">SEGMENTO ${idea.segmento}</span>` : ""}
      ${idea.categoria ? `<span class="badge b-purple">${idea.categoria}</span>` : ""}
      ${idea.tipo ? `<span class="badge b-blue">${idea.tipo}</span>` : ""}
    </div>
    ${idea.concept ? `<div class="concept-box"><div class="concept-lbl">CONCEPTO DEL PRODUCTO</div><div class="concept-txt">${idea.concept.replace(/</g,"&lt;")}</div></div>` : ""}
  </div>
  <div class="hero-foto">
...
<div class="info-grid">
  <div class="info-cell"><div class="info-lbl">PERFIL DE CLIENTE</div><div class="info-val">${idea.perfil || "—"}</div></div>
  <div class="info-cell"><div class="info-lbl">TIPO DE LANZAMIENTO</div><div class="info-val">${idea.tipo || "—"}</div></div>
  <div class="info-cell"><div class="info-lbl">TENDENCIA / INSPIRACIÓN</div><div class="info-val">${idea.trend || "—"}</div></div>
</div>

${(idea.insumos || idea.recipe) ? `
<div class="section">
  <div class="two-col">
    ${idea.insumos ? `<div><div class="sec-head"><div class="sec-icon" style="background:#8B5CF620">🧪</div><div class="sec-title" style="color:#8B5CF6">INSUMOS E INGREDIENTES</div></div><div class="sec-content">${idea.insumos.replace(/</g,"&lt;")}</div></div>` : ""}
    ${idea.recipe ? `<div><div class="sec-head"><div class="sec-icon" style="background:#F9731620">📋</div><div class="sec-title" style="color:#F97316">RECETA DETALLADA</div></div><div class="sec-content">${idea.recipe.replace(/</g,"&lt;")}</div></div>` : ""}
  </div>
</div>` : ""}

${idea.layout ? `
<div class="section">
  <div class="sec-head"><div class="sec-icon" style="background:#22C55E20">🏗️</div><div class="sec-title" style="color:#22C55E">PROCEDIMIENTO Y LAYOUT DE ARMADO</div></div>
  <div class="sec-content">${idea.layout.replace(/</g,"&lt;")}</div>
</div>` : ""}

${idea.notas ? `
<div class="section">
  <div class="sec-head"><div class="sec-icon" style="background:#3B82F620">📝</div><div class="sec-title" style="color:#3B82F6">NOTAS ADICIONALES</div></div>
  <div class="sec-content">${idea.notas.replace(/</g,"&lt;")}</div>
</div>` : ""}

${fotos.length > 1 ? `
<div class="section">
  <div class="sec-head"><div class="sec-icon" style="background:#3B82F620">📷</div><div class="sec-title" style="color:#3B82F6">REGISTRO FOTOGRÁFICO</div></div>
  <div class="fotos-grid">${fotos.map(f=>`<div class="foto-item"><img src="${f.url}"/></div>`).join("")}</div>
</div>` : ""}

<div class="footer">
  <div class="footer-brand">BIG★PONS — PRODUCT LAB</div>
  <div>${idea.name} · Ficha Técnica</div>
...
.btn-ol:hover{background:rgba(255,186,0,.1)}
...
.sec{background:#161616;border:1px solid #252525;border-radius:12px;padding:24px;margin-bottom:14px}
...
.phase-step{flex:1;padding:14px 10px;text-align:center;transition:background .2s;cursor:pointer}
  ...
  return (
    <div style={{display:"grid",gap:16}}>
      <div style={{background:"rgba(59,130,246,.08)",border:"1px solid rgba(59,130,246,.2)",borderRadius:10,padding:"12px 16px",display:"flex",gap:10,alignItems:"center"}}>
        ...
        <div>
          <div style={{fontSize:11,fontWeight:700,color:"#3B82F6"}}>FASE 1 — IDEA INICIAL</div>
          <div style={{fontSize:11,color:"#666"}}>Completá los datos básicos para evaluar la idea</div>
        </div>
      </div>
      ...
      <div><Lbl>Concepto y descripción</Lbl><textarea rows={5} placeholder="Qué es el producto, qué lo hace especial, por qué encaja con BIG PONS..." value={f.concept} onChange={e=>s("concept",e.target.value)}/></div>
  ...
  return (
    <div style={{display:"grid",gap:16}}>
      <div style={{background:"rgba(139,92,246,.08)",border:"1px solid rgba(139,92,246,.2)",borderRadius:10,padding:"12px 16px",display:"flex",gap:10,alignItems:"center"}}>
        <span style={{fontSize:18}}>🧪</span>
        <div>
          <div style={{fontSize:11,fontWeight:700,color:"#8B5CF6"}}>FASE 2 — PRUEBA DE PRODUCTO</div>
          <div style={{fontSize:11,color:"#666"}}>Cargá el detalle técnico del producto</div>
        </div>
      </div>
      <div><Lbl>Insumos — listado de ingredientes</Lbl>
        <textarea rows={7} placeholder={"- Pan brioche — 1 unidad (80g)\n- Medallón de carne — 180g\n- Queso cheddar — 2 fetas (30g)\n- Lechuga — 20g\n- Tomate — 2 rodajas"} value={f.insumos} onChange={e=>s("insumos",e.target.value)}/>
      </div>
      <div><Lbl>Receta detallada</Lbl>
        <textarea rows={8} placeholder={"1. Precalentar plancha a 220°C\n2. Formar medallón de 180g y aplastar\n3. Cocinar 2 min por lado\n4. Agregar queso y cubrir 30 seg\n5. Tostar pan 45 seg..."} value={f.recipe} onChange={e=>s("recipe",e.target.value)}/>
      </div>
      <div><Lbl>Procedimiento y Layout de armado</Lbl>
        <textarea rows={9} placeholder={"LAYOUT (de abajo hacia arriba):\n[ BASE DEL PAN ]\n[ SALSA — 15g ]\n[ LECHUGA — 20g ]\n[ TOMATE x2 ]\n[ MEDALLÓN + QUESO ]\n[ CEBOLLA CARAMELIZADA ]\n[ TAPA DEL PAN ]\n\nPROCEDIMIENTO:\n1. Tostar pan 45 seg...\n2. Aplicar salsa en base..."} value={f.layout} onChange={e=>s("layout",e.target.value)}/>
      </div>
      <div><Lbl>Notas adicionales</Lbl>
        <textarea rows={3} placeholder="Observaciones, restricciones, puntos de control de calidad, alérgenos..." value={f.notas} onChange={e=>s("notas",e.target.value)}/>
      </div>
      <div style={{display:"flex",gap:10,justifyContent:"flex-end",paddingTop:8,borderTop:`1px solid #252525`}}>
        <button className="btn-gh" onClick={onCancel}>Cancelar</button>
...
export default function BigPonsLab() {
  ...
  const addIdea = useCallback((data)=>{
    setIdeas(p=>[{id:Date.now(),createdAt:new Date().toISOString(),fase:"fase1",fotos:[],...data},...p]);
    setNewModal(false);
  }, []);
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0D0D0D",color:"#F0F0F0",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"32px 24px"}}>
        {!loaded && <div style={{textAlign:"center",padding:80}}><p style={{color:"#555"}} className="pulse">Cargando...</p></div>}
              ...
              return (
                <div key={idea.id} className="hcard card" onClick={()=>setSel(idea.id)} style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:16}}>
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    {idea.concept && <p style={{fontSize:13,color:"#666",lineHeight:1.5}}>{idea.concept.substring(0,180)}...</p>}
                  </div>
...
function IdeaDetail({idea, updateIdea, onBack}) {
  ...
  const aprobarFase2 = () => { updateIdea(idea.id,{fase:"fase3"}); setActiveTab("fase3"); };
  ...
  return (
    <div className="fu">
      <div style={{display:"flex",alignItems:"flex-start",gap:12,marginBottom:20,flexWrap:"wrap"}}>
        ...
        {faseNum === 3 && (
          <button className="btn-gold" onClick={()=>generarPDF(idea)}>⬇ Descargar PDF</button>
        )}
      </div>
          ...
          <div className="sec">
            ...
            {idea.concept && (
              <div style={{marginTop:16}}>
                <div style={{fontSize:11,fontWeight:700,color:"#555",marginBottom:8}}>CONCEPTO Y DESCRIPCIÓN</div>
                <pre style={{whiteSpace:"pre-wrap",fontSize:14,lineHeight:1.8,color:"#E0E0E0",fontFamily:"'DM Sans'"}}>{idea.concept}</pre>
              </div>
            )}
          </div>
          {idea.fase==="fase1" && (
          ...
          {faseNum < 2 ? <Locked text="Se habilita al aprobar la Fase 1."/> : (
            <div>
              <div className="sec">
                ...
                {idea.insumos ? <pre style={{whiteSpace:"pre-wrap",fontSize:14,lineHeight:1.8,color:"#E0E0E0",fontFamily:"'DM Sans'"}}>{idea.insumos}</pre>
                  : <p style={{color:"#555",fontSize:13}}>Sin insumos. Hacé clic en Editar para cargar.</p>}
              </div>
              <div className="sec">
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                ...
                {idea.recipe ? <pre style={{whiteSpace:"pre-wrap",fontSize:14,lineHeight:1.8,color:"#E0E0E0",fontFamily:"'DM Sans'"}}>{idea.recipe}</pre>
                  : <p style={{color:"#555",fontSize:13}}>Sin receta. Hacé clic en Editar para cargar.</p>}
              </div>
              <div className="sec">
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                ...
                {idea.layout ? <pre style={{whiteSpace:"pre-wrap",fontSize:14,lineHeight:1.8,color:"#E0E0E0",fontFamily:"'DM Sans'"}}>{idea.layout}</pre>
                  : <p style={{color:"#555",fontSize:13}}>Sin layout. Hacé clic en Editar para cargar.</p>}
              </div>
              {idea.notas && (
                <div className="sec">
                  <div style={{fontSize:12,fontWeight:700,color:"#3B82F6",letterSpacing:.5,marginBottom:12}}>📝 NOTAS ADICIONALES</div>
                  <pre style={{whiteSpace:"pre-wrap",fontSize:14,lineHeight:1.8,color:"#E0E0E0",fontFamily:"'DM Sans'"}}>{idea.notas}</pre>
                </div>
              )}
              {idea.fase==="fase2" && (
                <div style={{background:"rgba(255,186,0,.06)",border:`1px solid rgba(255,186,0,.2)`,borderRadius:12,padding:20,display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                  <div>
                    ...
                    <div style={{fontSize:13,color:"#666"}}>Al aprobar pasa a <strong style={{color:"#FFBA00"}}>Fase 3 — Listo para Presentación</strong>.</div>
                  </div>
                  <button className="btn-gold" onClick={aprobarFase2}>⭐ Aprobar → Fase 3</button>
                </div>
              )}
            </div>
          )}
      ...
      {activeTab==="fase3" && (
        <div>
          {faseNum < 3 ? <Locked text="Se habilita al aprobar la Fase 2."/> : (
            <div>
              <div style={{background:"rgba(255,186,0,.07)",border:`1px solid rgba(255,186,0,.2)`,borderRadius:12,padding:"16px 20px",marginBottom:20,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  <span style={{fontSize:24}}>⭐</span>
                  <div>
                    <div style={{fontSize:13,fontWeight:700,color:"#FFBA00"}}>PRODUCTO LISTO PARA PRESENTACIÓN</div>
                    <div style={{fontSize:12,color:"#666"}}>El PDF incluye toda la información cargada + fotos</div>
                  </div>
                </div>
              ...
              <div className="sec">
                <div style={{fontSize:12,fontWeight:700,color:"#3B82F6",letterSpacing:.5,marginBottom:16}}>📋 RESUMEN DEL PRODUCTO</div>
                <div style={{display:"grid",gap:10}}>
                  {[
                    ["Nombre",idea.name],["Categoría",idea.categoria],["Segmento",idea.segmento?`Segmento ${idea.segmento}`:""],
                    ["Tipo",idea.tipo],["Perfil de Cliente",idea.perfil],["Tendencia",idea.trend],
                  ].filter(([,v])=>v).map(([k,v])=>(
                    <div key={k} style={{display:"flex",gap:16,padding:"8px 0",borderBottom:`1px solid #1E1E1E`}}>
                      <span style={{fontSize:11,fontWeight:700,color:"#555",minWidth:140,flexShrink:0}}>{k.toUpperCase()}</span>
                      <span style={{fontSize:14}}>{v}</span>
              ...
              <div className="sec">
                ...
                {(!idea.fotos || idea.fotos.length===0) ? (
                  <div style={{border:`2px dashed #2E2E2E`,borderRadius:10,padding:"40px 20px",textAlign:"center",cursor:"pointer"}} onClick={()=>fileRef.current?.click()}>
                    ...
                    <div style={{fontSize:14,color:"#555"}}>Hacé clic para subir fotos del producto</div>
