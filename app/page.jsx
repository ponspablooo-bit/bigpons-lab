...
const SYS_RECIPE = `Sos el chef de desarrollo de BIG PONS, hamburguesería premium americana con locales en Argentina. Hacés recetas precisas, escalables, listas para producción en fast food premium. Incluís gramajes exactos, tiempos, temperatura, notas de calidad y puntos de control. Respondé en español de Argentina.`;
const SYS_LAYOUT = `Sos el jefe de operaciones de BIG PONS. Desarrollás procedimientos de armado paso a paso con layout visual en texto (de abajo hacia arriba) para que cualquier empleado pueda armar el producto correctamente. Incluís tiempos, temperaturas y puntos de control de calidad. Respondé en español de Argentina.`;
const SYS_FICHA = `Sos el responsable de producto de BIG PONS, hamburguesería premium americana. Generás fichas técnicas completas y profesionales de productos gastronómicos, listas para ser presentadas al Gerente General. Incluís todos los detalles comerciales, técnicos y operativos. Respondé en español de Argentina.`;

// ── SUPABASE ──────────────────────────────────────────────────────────────────
async function sLoad() {
  try { return await loadIdeas(); } catch { return []; }
}
async function sSave(ideas) {
  try { await saveIdeas(ideas); } catch {}
}
...
.btn-green{background:#22C55E;color:#000;border:none;padding:10px 22px;border-radius:8px;font-family:'DM Sans',sans-serif;font-weight:700;font-size:14px;cursor:pointer;transition:all .2s;display:inline-flex;align-items:center;gap:8px;white-space:nowrap}
...
.divider{height:1px;background:#252525;margin:20px 0}
...
.phase-step{flex:1;padding:14px 10px;text-align:center;position:relative;transition:background .2s}
...
function Fase1Form({ initial, onSave, onCancel }) {
  ...
  return (
    <div style={{ display:"grid", gap:16 }}>
      ...
      <div><Lbl>Concepto y descripción del producto</Lbl><textarea rows={5} placeholder="Describí el producto: qué es, qué lo hace especial, la inspiración, por qué encaja con BIG PONS..." value={f.concept} onChange={e=>s("concept",e.target.value)} /></div>
      <div><Lbl>Perfil de cliente objetivo</Lbl><input placeholder="Ej: Adultos 25-40, foodie premium, busca experiencias nuevas" value={f.perfil} onChange={e=>s("perfil",e.target.value)} /></div>
  ...
  return (
    <div style={{ display:"grid", gap:16 }}>
      <div style={{ background:"rgba(139,92,246,.08)", border:"1px solid rgba(139,92,246,.2)", borderRadius:10, padding:"12px 16px", display:"flex", gap:10, alignItems:"center" }}>
        <span style={{ fontSize:18 }}>🧪</span>
        <div><div style={{ fontSize:11, fontWeight:700, color:"#8B5CF6", letterSpacing:.5 }}>FASE 2 — PRUEBA DE PRODUCTO</div><div style={{ fontSize:11, color:"#666" }}>Cargá o editá el detalle técnico del producto</div></div>
      </div>
      <div>
        <Lbl>Insumos — Listado de ingredientes</Lbl>
        <textarea rows={7} placeholder={"Listá todos los ingredientes con cantidad por porción. Ej:\n- Pan brioche — 1 unidad (80g)\n- Medallón de carne — 180g\n- Queso cheddar — 2 fetas (30g)\n- Lechuga — 20g"} value={f.insumos} onChange={e=>s("insumos",e.target.value)} />
      </div>
      <div>
        <Lbl>Receta detallada</Lbl>
        <textarea rows={8} placeholder={"Procedimiento paso a paso. Ej:\n1. Precalentar plancha a 220°C\n2. Formar medallón de 180g y aplastar\n3. Cocinar 2 min por lado\n4. Agregar queso y cubrir 30 seg..."} value={f.recipe} onChange={e=>s("recipe",e.target.value)} />
      </div>
      <div>
        <Lbl>Procedimiento y Layout de armado</Lbl>
        <textarea rows={9} placeholder={"LAYOUT (de abajo hacia arriba):\n[ BASE DEL PAN ]\n[ SALSA — 15g ]\n[ LECHUGA — 20g ]\n[ TOMATE x2 ]\n[ MEDALLÓN + QUESO ]\n[ CEBOLLA CARAMELIZADA ]\n[ TAPA DEL PAN ]\n\nPROCEDIMIENTO:\n1. Tostar pan en plancha 45 seg...\n2. Aplicar salsa en base...\n3. Montar según layout..."} value={f.layout} onChange={e=>s("layout",e.target.value)} />
      </div>
      <div style={{ display:"flex", gap:10, justifyContent:"flex-end", paddingTop:8, borderTop:`1px solid #252525` }}>
        <button className="btn-gh" onClick={onCancel}>Cancelar</button>
...
function IdeasTab({ ideas, updateIdea, deleteIdea }) {
  ...
  if (sel && idea) {
    return <IdeaDetail idea={idea} updateIdea={updateIdea} onBack={()=>setSel(null)} />;
  }

  return (
    <div className="fu">
      <SectionTitle sub="Todas las ideas del equipo — desde el concepto inicial hasta el producto listo">IDEAS & DESARROLLO</SectionTitle>
        ...
        : (
          <div style={{ display:"grid", gap:12 }}>
            {filtered.map(idea=>{
              ...
              return (
                <div key={idea.id} className="hcard card" onClick={()=>setSel(idea.id)}
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:10 }}>
                      ...
                      {idea.fotos?.length>0 && <Tag color="#22C55E">📷 {idea.fotos.length} foto{idea.fotos.length!==1?"s":""}</Tag>}
                    </div>
...
function IdeaDetail({ idea, updateIdea, onBack }) {
  ...
  const [loading, setLoading] = useState(null); // 'recipe' | 'layout' | 'ficha'
  ...
  const aprobarFase2 = async () => {
    ...
    setActiveTab("fase3");
    // Auto-generate ficha if not exists
    if (!idea.ficha) {
      setLoading("ficha");
      try {
        const r = await callClaude(
          `Generá una ficha técnica completa y profesional para presentar al Gerente General de BIG PONS:\n\nNombre: ${idea.name}\nCategoría: ${idea.categoria||""}\nSegmento: ${idea.segmento||""}\nTipo: ${idea.tipo||""}\nConcepto: ${idea.concept||""}\nPerfil de cliente: ${idea.perfil||""}\nInsumos: ${idea.insumos||""}\nReceta: ${idea.recipe||""}\nLayout de armado: ${idea.layout||""}\n\nIncluí:\n**NOMBRE COMERCIAL DEL PRODUCTO:**\n**DESCRIPCIÓN PARA MENÚ:** (texto atractivo para el cliente)\n**CATEGORÍA Y SEGMENTO:**\n**PERFIL DE CLIENTE OBJETIVO:**\n**INGREDIENTES Y GRAMAJES (por porción):**\n**RECETA Y PROCEDIMIENTO:**\n**LAYOUT DE ARMADO:**\n**PRECIO DE VENTA SUGERIDO (ARS):**\n**CMV ESTIMADO (%):**\n**OBJETIVO DEL PRODUCTO:** (volumen o rentabilidad)\n**ALÉRGENOS:**\n**NOTAS PARA PRODUCCIÓN Y OPERACIONES:**\n**POTENCIAL DE VENTAS:**\n**RECOMENDACIÓN FINAL:**`,
          SYS_FICHA
        );
        updateIdea(idea.id, { fase:"fase3", ficha:r });
      } catch { alert("Error generando la ficha. Podés generarla manualmente desde la Fase 3."); }
      setLoading(null);
    }
  };
  ...
  const genFicha = async () => {
    ...
    try {
      const r = await callClaude(
        `Ficha técnica completa para BIG PONS:\nNombre: ${idea.name}\nCategoría: ${idea.categoria||""}\nSegmento: ${idea.segmento||""}\nTipo: ${idea.tipo||""}\nConcepto: ${idea.concept||""}\nPerfil: ${idea.perfil||""}\nInsumos: ${idea.insumos||""}\nReceta: ${(idea.recipe||"").substring(0,400)}\nLayout: ${(idea.layout||"").substring(0,400)}\n\n**NOMBRE COMERCIAL DEL PRODUCTO:**\n**DESCRIPCIÓN PARA MENÚ:**\n**CATEGORÍA Y SEGMENTO:**\n**PERFIL DE CLIENTE:**\n**INGREDIENTES Y GRAMAJES:**\n**RECETA Y PROCEDIMIENTO:**\n**LAYOUT DE ARMADO:**\n**PRECIO DE VENTA SUGERIDO (ARS):**\n**CMV ESTIMADO (%):**\n**OBJETIVO:** (volumen o rentabilidad)\n**ALÉRGENOS:**\n**NOTAS PARA PRODUCCIÓN:**\n**POTENCIAL DE VENTAS:**\n**RECOMENDACIÓN FINAL:**`,
        SYS_FICHA
      );
      updateIdea(idea.id, { ficha:r });
    } catch { alert("Error generando ficha."); }
    setLoading(null);
  };
  ...
  const deleteFoto = (fotoId) => {
    updateIdea(idea.id, { fotos:(idea.fotos||[]).filter(f=>f.id!==fotoId) });
  };
      ...
      {FASES.map((f, i) => {
        ...
        return (
          <div key={f.id} style={{ display:"contents" }}>
            <div className={`phase-step ${isActive?"active":""} ${isDone?"done":""}`}
              style={{ cursor:"pointer" }} onClick={()=>setActiveTab(f.id)}>
  ...
  return (
    <div className="fu">
        ...
        <div>
          <div className="section">
            ...
            <div style={{ display:"grid", gap:10 }}>
              {[["Categoría",idea.categoria],["Tipo de Lanzamiento",idea.tipo],["Segmento",idea.segmento?`Segmento ${idea.segmento}`:""],["Perfil de Cliente",idea.perfil],["Tendencia / Inspiración",idea.trend]].filter(([,v])=>v).map(([k,v])=>(
                <div key={k} style={{ display:"flex", gap:16, padding:"8px 0", borderBottom:`1px solid #1E1E1E` }}>
                  <span style={{ fontSize:11, fontWeight:700, color:"#555", minWidth:160, flexShrink:0 }}>{k.toUpperCase()}</span>
          ...
          {idea.fase==="fase1" && (
            <div style={{ background:"rgba(139,92,246,.07)", border:`1px solid rgba(139,92,246,.2)`, borderRadius:12, padding:20, display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, flexWrap:"wrap" }}>
              <div>
                ...
                <div style={{ fontSize:13, color:"#666" }}>Al aprobar, pasa a <strong style={{ color:"#8B5CF6" }}>Fase 2 — Prueba de Producto</strong> donde se desarrolla el detalle técnico.</div>
              </div>
      ...
      {activeTab==="fase2" && (
        <div>
          {faseNum < 2 ? <Locked text="Esta fase se habilita al aprobar la Fase 1." /> : (
            <div>
              {/* Insumos */}
              <div className="section">
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
                  <div style={{ fontSize:12, fontWeight:700, color:"#8B5CF6", letterSpacing:.5 }}>🧪 INSUMOS — LISTADO DE INGREDIENTES</div>
                ...
                {idea.insumos ? <pre style={{ whiteSpace:"pre-wrap", fontSize:14, lineHeight:1.8, color:"#E0E0E0", fontFamily:"'DM Sans'" }}>{idea.insumos}</pre>
                  : <p style={{ color:"#555", fontSize:13 }}>Sin insumos cargados. Hacé clic en Editar para agregar.</p>}
              </div>
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
                  ...
                  <div style={{ display:"flex", gap:8 }}>
                    <button className="btn-gold" disabled={!!loading} onClick={genRecipe} style={{ fontSize:12, padding:"6px 14px" }}>
                      {loading==="recipe" ? <><Spinner size={12} color="#000" />Generando...</> : (idea.recipe?"🔄 Regenerar IA":"🤖 Generar con IA")}
                    </button>
                    <button className="btn-gh" onClick={()=>setEditFase2(true)}>✏️ Editar</button>
                  </div>
                </div>
                {idea.recipe ? <pre style={{ whiteSpace:"pre-wrap", fontSize:14, lineHeight:1.8, color:"#E0E0E0", fontFamily:"'DM Sans'" }}>{idea.recipe}</pre>
                  : <p style={{ color:"#555", fontSize:13 }}>Sin receta. Generala con IA o cargala manualmente.</p>}
              </div>
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
                  ...
                  <div style={{ display:"flex", gap:8 }}>
                    <button className="btn-gold" disabled={!!loading} onClick={genLayout} style={{ fontSize:12, padding:"6px 14px" }}>
                      {loading==="layout" ? <><Spinner size={12} color="#000" />Generando...</> : (idea.layout?"🔄 Regenerar IA":"🤖 Generar con IA")}
                    </button>
                    <button className="btn-gh" onClick={()=>setEditFase2(true)}>✏️ Editar</button>
                  </div>
                </div>
                {idea.layout ? <pre style={{ whiteSpace:"pre-wrap", fontSize:14, lineHeight:1.8, color:"#E0E0E0", fontFamily:"'DM Sans'" }}>{idea.layout}</pre>
                  : <p style={{ color:"#555", fontSize:13 }}>Sin layout. Generalo con IA o cargalo manualmente.</p>}
              </div>
              ...
              {idea.fase==="fase2" && (
                <div style={{ background:"rgba(255,186,0,.06)", border:`1px solid rgba(255,186,0,.2)`, borderRadius:12, padding:20, display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, flexWrap:"wrap" }}>
                  <div>
                    ...
                    <div style={{ fontSize:13, color:"#666" }}>Al aprobar, el producto pasa a <strong style={{ color:"#FFBA00" }}>Fase 3 — Listo para Presentación</strong> y se genera la ficha completa automáticamente.</div>
                  </div>
                  <button className="btn-gold" onClick={aprobarFase2} disabled={!!loading}>
                    {loading==="ficha" ? <><Spinner size={14} color="#000" />Generando ficha...</> : "⭐ Aprobar → Fase 3"}
                  </button>
                </div>
              )}
            </div>
      ...
      {activeTab==="fase3" && (
        <div>
          {faseNum < 3 ? <Locked text="Esta fase se habilita al aprobar la Fase 2 — Prueba de Producto." /> : (
            <div>
              {/* Badge */}
              <div style={{ background:"rgba(255,186,0,.07)", border:`1px solid rgba(255,186,0,.2)`, borderRadius:12, padding:"14px 20px", marginBottom:20, display:"flex", alignItems:"center", gap:12 }}>
                <span style={{ fontSize:24 }}>⭐</span>
                <div>
                  <div style={{ fontSize:13, fontWeight:700, color:"#FFBA00" }}>PRODUCTO LISTO PARA PRESENTACIÓN</div>
                  <div style={{ fontSize:12, color:"#666" }}>Ficha técnica completa + fotos del producto</div>
                </div>
              </div>
              ...
              <div className="section">
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
                  <div style={{ fontSize:12, fontWeight:700, color:"#FFBA00", letterSpacing:.5 }}>📄 FICHA TÉCNICA COMPLETA</div>
                  <button className="btn-gold" disabled={!!loading} onClick={genFicha} style={{ fontSize:12, padding:"6px 14px" }}>
                    {loading==="ficha" ? <><Spinner size={12} color="#000" />Generando...</> : (idea.ficha?"🔄 Regenerar ficha":"🤖 Generar ficha con IA")}
                  </button>
                </div>
                {idea.ficha
                  ? <pre style={{ whiteSpace:"pre-wrap", fontSize:14, lineHeight:1.9, color:"#E0E0E0", fontFamily:"'DM Sans'" }}>{idea.ficha}</pre>
                  : <p style={{ color:"#555", fontSize:13 }}>Sin ficha generada todavía. Hacé clic en "Generar ficha con IA".</p>
                }
              </div>
              ...
              <div className="section">
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
                  ...
                  <button className="btn-ol" style={{ fontSize:12 }} onClick={()=>fileRef.current?.click()}>
                    📷 Agregar fotos
                  </button>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(200px, 1fr))", gap:12 }}>
                    {idea.fotos.map(foto=>(
                      <div key={foto.id} style={{ position:"relative", borderRadius:10, overflow:"hidden", border:`1px solid #252525`, aspectRatio:"4/3" }}>
                        ...
                        <button onClick={()=>deleteFoto(foto.id)}
                          style={{ position:"absolute", top:6, right:6, background:"rgba(0,0,0,.8)", border:"none", color:"#EF4444", width:26, height:26, borderRadius:"50%", cursor:"pointer", fontSize:13, display:"flex", alignItems:"center", justifyContent:"center" }}>
                          ✕
                        </button>
                      </div>
                    ))}
                    <div style={{ border:`2px dashed #2E2E2E`, borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", minHeight:120, aspectRatio:"4/3" }} onClick={()=>fileRef.current?.click()}>
                      <div style={{ textAlign:"center", color:"#444" }}>
                        ...
                        <div style={{ fontSize:12 }}>Agregar más</div>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {faseIdeas.map(idea=>(
                  <div key={idea.id} style={{ background:"#1A1A1A", border:`1px solid #2E2E2E`, borderRadius:8, padding:10 }}>
                    ...
                    <div style={{ display:"flex", gap:5, flexWrap:"wrap", marginBottom:8 }}>
                      ...
                      {idea.ficha && <span style={{ fontSize:9, color:"#FFBA00", fontWeight:700 }}>✓FICHA</span>}
